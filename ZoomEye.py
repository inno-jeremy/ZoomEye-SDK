...
def get_search_results(token, dork, facet="", search_type="host", page=1):
    ...
    response = requests.get(
        base_url + "/" + search_type + "/search?query=" + dork + "&facet=" + facet + "&page=" + str(page),
        headers=header)
    response.close()
...
def classify_hosts_results(result, para=['banner'], keyword="", show_print=True, show_return=True):
    '''
    ...
    for result_num in range(len(result["matches"])):  # 获取总结果数。
        if keyword and not keyword in result["matches"][result_num]["portinfo"][
            "banner"]:  # 如果zoomeye返回的结果中总有那么一两个结果不符合我们的需求，并且可以通过banner来识别它们，那么便可以在“keyword”中添加关键字，来选取符合自己需求的结果。
            continue

        if show_print:
            print "ip:", result["matches"][result_num]["ip"], "\n", "port:", result["matches"][result_num]["portinfo"][
                "port"]  # 打印IP地址,端口。
            for para_num in range(len(para)):  # 获取形参para列表的元素个数,打印对应信息。
                try:
        ...
        if show_return:
            ...
            for para_num in range(len(para)):  # 获取形参para列表的元素个数,添加至临时列表。
                try:
                    if para[para_num] in geoinfo:
...
def classify_web_results(result, para=['headers'], keyword="", show_print=True, show_return=True):
    '''
    对来自get_search_result函数的数据进行处理————直接显示(print)一份,供使用者阅读；以列表形式返回(return)一份,供poc等程序使用。
    '''
    ...
    if result["total"] % 10 == 0:
        ...
        print "Total pages:", result["total"] / 10 + 1, "\n"
    for result_num in range(len(result["matches"])):  # 获取总结果数。
        if keyword and not keyword in result["matches"][result_num]["headers"]:   # 如果zoomeye返回的结果中总有那么一两个结果不符合我们的需求，并且可以通过headers来识别它们，那么便可以在“keyword”中添加关键字，来选取符合自己需求的结果。
            continue
